package com.birdwind.init.view.customer.maskEditText;
...
public class MaskEditText extends AppCompatEditText implements TextWatcher {
    ...
    private final OnEditorActionListener onEditorActionListener = (v, actionId, event) -> true;
    ...
    private boolean keepOriginalHint;
    ...
    public MaskEditText(Context context, AttributeSet attrs) {
        ...
        keepOriginalHint = attributes.getBoolean(R.styleable.MaskEditText_keep_original_hint, false);
        ...
        mask = TextUtils.isEmpty(mask) ? "" : mask;
        allowedChars = TextUtils.isEmpty(allowedChars) ? "" : allowedChars;
        deniedChars = TextUtils.isEmpty(deniedChars) ? "" : deniedChars;
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        setText(text);
        // LogUtils.d("onRestoreInstanceState: " + text);
    }

    /**
     * @param listener - its onFocusChange() method will be called before performing MaskedEditText operations, related
     *                 to this event.
     */
    @Override
    ...
    /**
     * Generates positions for values characters. For instance: Input data: mask = "+7(###)###-##-## After method
     * execution: rawToMask = [3, 4, 5, 6, 8, 9, 11, 12, 14, 15] maskToRaw = [-1, -1, -1, 0, 1, 2, -1, 3, 4, 5, -1, 6,
     * 7, -1, 8, 9] charsInMask = "+7()- " (and space, yes)
     */
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        if (!editingOnChanged && editingBefore) {
            ...
            if (count > 0 && !TextUtils.isEmpty(mask)) {
    ...
    public void afterTextChanged(Editable s) {
        if (!editingAfter && editingBefore && editingOnChanged && rawText != null) {
            editingAfter = true;
    ...
    protected void onSelectionChanged(int selStart, int selEnd) {
        // On Android 4+ this method is being called more than 1 time if there is a hint in the EditText, what moves the
        // cursor to left
    ...
    private CharSequence makeMaskedTextWithHint() {
        ...
        if (!keepOriginalHint) {
            ...
            if (hasHint()) {
                ...
                ssb.append(hint);
                ssb.setSpan(new ForegroundColorSpan(getCurrentHintTextColor()), 0, hint.length(), 0);
            }
        }
        return ssb;
    }
    ...
    private Range calculateRange(int start, int end) {
        ...
        if (end == mask.length() && rawText != null) {
