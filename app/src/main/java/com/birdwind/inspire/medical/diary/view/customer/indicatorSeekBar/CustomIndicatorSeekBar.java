package com.birdwind.inspire.medical.diary.view.customer.indicatorSeekBar;
 ...
 * <p>
 * A custom SeekBar on Android, which can be changed the size , color , thumb drawable , tick drawable , texts ,
 * indicator; also can show an indicator view above SeekBar when seeking.
 */

public class CustomIndicatorSeekBar extends View {
    ...
    private boolean mUserSeekable;// true if the user can seek to change the progress,otherwise only can be changed by
                                  // setProgress().
    ...
    private float mTouchXCache;

    public CustomIndicatorSeekBar(Context context) {
        this(context, null);
    }

    public CustomIndicatorSeekBar(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public CustomIndicatorSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * <p>
     * IndicatorSeekBar .with(getContext()) .max(50) .min(10) ... .build();
     */
    CustomIndicatorSeekBar(Builder builder) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mIsFloatProgress =
            ta.getBoolean(R.styleable.IndicatorSeekBar_isb_progress_value_float, builder.progressValueFloat);
        ...
        mOnlyThumbDraggable =
            ta.getBoolean(R.styleable.IndicatorSeekBar_isb_only_thumb_draggable, builder.onlyThumbDraggable);
        ...
        // track
        mBackgroundTrackSize = ta.getDimensionPixelSize(R.styleable.IndicatorSeekBar_isb_track_background_size,
            builder.trackBackgroundSize);
        mProgressTrackSize =
            ta.getDimensionPixelSize(R.styleable.IndicatorSeekBar_isb_track_progress_size, builder.trackProgressSize);
        mBackgroundTrackColor =
            ta.getColor(R.styleable.IndicatorSeekBar_isb_track_background_color, builder.trackBackgroundColor);
        mProgressTrackColor =
            ta.getColor(R.styleable.IndicatorSeekBar_isb_track_progress_color, builder.trackProgressColor);
        mTrackRoundedCorners =
            ta.getBoolean(R.styleable.IndicatorSeekBar_isb_track_rounded_corners, builder.trackRoundedCorners);
        // thumb
        ...
        mShowTickMarksType =
            ta.getInt(R.styleable.IndicatorSeekBar_isb_show_tick_marks_type, builder.showTickMarksType);
        mTickMarksSize =
            ta.getDimensionPixelSize(R.styleable.IndicatorSeekBar_isb_tick_marks_size, builder.tickMarksSize);
        initTickMarksColor(ta.getColorStateList(R.styleable.IndicatorSeekBar_isb_tick_marks_color),
            builder.tickMarksColor);
        mTickMarksDrawable = ta.getDrawable(R.styleable.IndicatorSeekBar_isb_tick_marks_drawable);
        mTickMarksSweptHide =
            ta.getBoolean(R.styleable.IndicatorSeekBar_isb_tick_marks_swept_hide, builder.tickMarksSweptHide);
        mTickMarksEndsHide =
            ta.getBoolean(R.styleable.IndicatorSeekBar_isb_tick_marks_ends_hide, builder.tickMarksEndsHide);
        // tickTexts
        mShowTickText = ta.getBoolean(R.styleable.IndicatorSeekBar_isb_show_tick_texts, builder.showTickText);
        mTickTextsSize =
            ta.getDimensionPixelSize(R.styleable.IndicatorSeekBar_isb_tick_texts_size, builder.tickTextsSize);
        initTickTextsColor(ta.getColorStateList(R.styleable.IndicatorSeekBar_isb_tick_texts_color),
            builder.tickTextsColor);
        mTickTextsCustomArray = ta.getTextArray(R.styleable.IndicatorSeekBar_isb_tick_texts_array);
        initTextsTypeface(ta.getInt(R.styleable.IndicatorSeekBar_isb_tick_texts_typeface, -1),
        ...
        mIndicatorTextSize =
            ta.getDimensionPixelSize(R.styleable.IndicatorSeekBar_isb_indicator_text_size, builder.indicatorTextSize);
        mIndicatorTextColor =
            ta.getColor(R.styleable.IndicatorSeekBar_isb_indicator_text_color, builder.indicatorTextColor);
    ...
    private void measureTickTextsBonds() {
        if (needDrawText()) {
            ...
            mTickTextsHeight = mRect.height() + SizeUtils.dp2px(mContext, 3);// with the gap(3dp) between tickTexts and
                                                                             // track.
        }
    }
    ...
    private void refreshSeekBarLocation() {
        ...
        if (needDrawText()) {
            mTextPaint.getTextBounds("j", 0, 1, mRect);
            mTickTextY = mPaddingTop + mCustomDrawableMaxHeight + Math.round(mRect.height() - mTextPaint.descent())
                + SizeUtils.dp2px(mContext, 3);
...
//            if (progress > mEnableValue) {
//                float oProgress = (mBackgroundTrack.left - mPaddingLeft) * getAmplitude() / mSeekLength + mMin;
//                progress = mEnableValue + oProgress;
//            }
//            LogUtils.d("輸出", String.valueOf(progress));
        ...
        if (mCustomTrackSectionColorResult) {// the track has custom the section track color
            ...
            for (int i = 0; i < sectionSize; i++) {
                ...
                if (i < thumbPosFloat && thumbPosFloat < (i + 1)) {
                    ...
                    canvas.drawLine(mTickMarksX[i], mProgressTrack.top, thumbCenterX, mProgressTrack.bottom,
                        mStockPaint);
                    mStockPaint.setStrokeWidth(getRightSideTrackSize());
                    canvas.drawLine(thumbCenterX, mProgressTrack.top, mTickMarksX[i + 1], mProgressTrack.bottom,
                        mStockPaint);
                } else {
                    if (i < thumbPosFloat) {
                        mStockPaint.setStrokeWidth(getLeftSideTrackSize());
                    ...
                    canvas.drawLine(mTickMarksX[i], mProgressTrack.top, mTickMarksX[i + 1], mProgressTrack.bottom,
                        mStockPaint);
                }
            }
        } else {
        ...
        for (int i = 0; i < mTickMarksX.length; i++) {
            ...
            if (mShowTickMarksType == TickMarkType.OVAL) {
                ...
                canvas.drawRect(mTickMarksX[i] - rectWidth, mProgressTrack.top - dividerTickHeight / 2.0f,
                    mTickMarksX[i] + rectWidth, mProgressTrack.top + dividerTickHeight / 2.0f, mStockPaint);
    ...
    private void drawTickTexts(Canvas canvas) {
        ...
        for (int i = 0; i < mTickTextsArr.length; i++) {
            ...
            if (i == 0) {
                canvas.drawText(mTickTextsArr[index], mTextCenterX[i] + mTickTextsWidth[index] / 2.0f, mTickTextY,
                    mTextPaint);
    ...
    private void drawThumb(Canvas canvas) {
        ...
        float thumbCenterX = getThumbCenterX();
        if (mThumbDrawable != null) {// check user has set thumb drawable or not.ThumbDrawable first, thumb color for
                                     // later.
     ...
     * <p>
     * <p>
     * NOTICE: make sure the format of color selector you set is right. int[][] states = colorStateList.getStates(); (1)
     * if the states.length == 1,the way you set the thumb color like : app:isb_thumb_color="#XXXXXX" or
     ...
     * <item android:color="#555555" android:state_pressed="true" /> <!--this color is for thumb which is at pressing
     * status--> <item android:color="#555555"/> <!--for thumb which is at normal status--> </selector>
     * <p>
     ...
     * <p>
     * NOTICE: make sure the format of color selector you set is right. int[][] states = colorStateList.getStates(); (1)
     * if the states.length == 1,the way you set the tick marks' color like : app:isb_tick_marks_color="#XXXXXX" or
     ...
     * <p>
     * <?xml version="1.0" encoding="utf-8"?> <selector xmlns:android="http://schemas.android.com/apk/res/android">
     * <item android:color="#555555" android:state_selected="true" /> <!--this color is for marks those are at left side
     * of thumb--> <item android:color="#555555"/> <!--for marks those are at right side of thumb--> </selector>
     * <p>
     ...
     * <p>
     * NOTICE: make sure the format of color selector you set is right. int[][] states = colorStateList.getStates(); (1)
     * if the states.length == 1,the way you set the tick texts' color like : app:isb_tick_text_color="#XXXXXX" or
     ...
     * <p>
     * <?xml version="1.0" encoding="utf-8"?> <selector xmlns:android="http://schemas.android.com/apk/res/android">
     * <item android:color="#555555" android:state_selected="true" /> <!--this color is for texts those are at left side
     * of thumb--> <item android:color="#FF4081" android:state_hovered="true" /> <!--for thumb text-->
     * <item android:color="#555555"/> <!--for texts those are at right side of thumb--> </selector>
     * <p>
     * (3) if the states.length == other, the way you set is not support.
     */
    ...
    /**
     * initial both the tick texts' and thumb text's typeface,just has 4 type to choose, but you can set the CUSTOM
     * typeface you want by java code.
     * <p>
     * usage like: indicatorSeekbar.customTickTextsTypeface(Typeface.xxx);
     */
     ...
     * <p>
     * <p>
     * NOTICE: make sure the format of drawable selector file you set is right. int stateCount =
     * listDrawable.getStateCount(); (1) if the drawable instanceof BitmapDrawable,the way you set like :
     * app:isb_thumb_drawable="@drawable/ic_launcher"
     * <p>
     * (2) if the drawable instanceof StateListDrawable,the way you set like :
     * app:isb_thumb_drawable="@drawable/selector_thumb_drawable". the file(located at res/drawable/)'s format should
     * like:
     * <p>
     * <?xml version="1.0" encoding="utf-8"?> <selector xmlns:android="http://schemas.android.com/apk/res/android">
     * <item android:drawable="@drawable/ic_launcher" android:state_pressed="true" /> <!--this drawable is for thumb
     * when pressing--> <item android:drawable="@drawable/ic_launcher_round" /> <!--for thumb when normal--> </selector>
     */
     ...
     * <p>
     * (2) if the drawable instanceof StateListDrawable,the way you set like :
     * app:isb_tick_marks_drawable="@drawable/selector_thumb_drawable". the file(located at res/drawable/)'s format
     * should like:
     * <p>
     * <?xml version="1.0" encoding="utf-8"?> <selector xmlns:android="http://schemas.android.com/apk/res/android">
     * <item android:drawable="@drawable/ic_launcher" android:state_selected="true" /> <!--this drawable is for
     * thickMarks which thumb swept--> <item android:drawable="@drawable/ic_launcher_round" /> <!--for thickMarks which
     * thumb haven't reached--> </selector>
     */
    ...
    private void initIndicatorContentView() {
        ...
        if (mIndicator == null) {
            mIndicator = new Indicator(mContext, this, mIndicatorColor, mShowIndicatorType, mIndicatorTextSize,
                mIndicatorTextColor, mIndicatorContentView, mIndicatorTopContentView);
    ...
    private void apply(Builder builder) {
        ...
        this.mEnableValue = builder.enabledValue;
    ...
    /**
     * Sets the current progress to the specified value.also, if the seek bar's tick'count is larger than 2,the progress
     * will adjust to the closest tick's progress auto.
     *
     ...
     * Set the min value for SeekBar
     *
     * @param value the min value , if is larger than max, will set to max.
     */
    public synchronized void setEnableValue(float value) {
        this.mEnableValue = value;
        // initProgressRangeValue();
        // collectTicksInfo();
        // refreshSeekBarLocation();
        // invalidate();
    ...
    // <?xml version="1.0" encoding="utf-8"?>
    // <selector xmlns:android="http://schemas.android.com/apk/res/android">
    // <item android:color="@color/colorAccent" android:state_pressed="true" /> <!--this color is for thumb which is at
    // pressing status-->
    ...
    // <?xml version="1.0" encoding="utf-8"?>
    // <selector xmlns:android="http://schemas.android.com/apk/res/android">
    // <item android:color="@color/colorAccent" android:state_selected="true" /> <!--this color is for marks those are
    // at left side of thumb-->
    ...
    // <?xml version="1.0" encoding="utf-8"?>
    // <selector xmlns:android="http://schemas.android.com/apk/res/android">
    // <item android:color="@color/colorAccent" android:state_selected="true" /> <!--this color is for texts those are
    // at left side of thumb-->
    ...
    /**
     * The specified scale for the progress value, make sure you had chosen the float progress type
     * <p>
     * such as: scale = 3; progress: 1.78627347 to 1.786 scale = 4; progress: 1.78627347 to 1.7863
     * <p>
    ...
    /**
     * Set a format string with placeholder ${PROGRESS} or ${TICK_TEXT} to IndicatorSeekBar, the indicator's text would
     * change. For example: seekBar.setIndicatorTextFormat("${PROGRESS} %"); seekBar.setIndicatorTextFormat("${PROGRESS}
     * miles"); seekBar.setIndicatorTextFormat("I am ${TICK_TEXT}%");
     * <p>
     * make sure you have custom and show the tick text before you use ${TICK_TEXT}% , otherwise will be shown a ""
     * value.
     * <p>
     ...
     * <p>
     * public boolean collectSectionTrackColor(int[] colorIntArr) { colorIntArr[0] =
     * getResources().getColor(R.color.color_blue); colorIntArr[1] = getResources().getColor(R.color.color_gray);
     * colorIntArr[2] = Color.parseColor("#FFFF00"); ...... return true; // True if apply color , otherwise no change. }
     * });
     *
    ...
    /**
     * Replace the number ticks' texts with your's by String[]. Usually, the text array's length your set should equals
     * seek bar's tickMarks' count.
     *
