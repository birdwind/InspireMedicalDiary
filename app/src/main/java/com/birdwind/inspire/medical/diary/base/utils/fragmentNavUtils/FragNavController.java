...
public class FragNavController {
    // Declare the constants There is a maximum of 5 tabs, this is per Material Design's Bottom Navigation's design
    // spec.
    ...
    public void switchTab(@TabIndex int index, @Nullable FragNavTransactionOptions transactionOptions)
        ...
        if (index >= mFragmentStacks.size()) {
            throw new IndexOutOfBoundsException("Can't switch to a tab that hasn't been initialized, " + "Index : "
                + index + ", current stack size : " + mFragmentStacks.size()
    ...
    public void pushFragment(@Nullable Fragment fragment, @Nullable FragNavTransactionOptions transactionOptions) {
        if (fragment != null && mSelectedTabIndex != NO_TAB) {
            ...
            // 測試用code
            new Handler().postDelayed(() -> executePendingTransactions(), 3000);
            // executePendingTransactions();
    ...
    /**
     * Pop the current stack until a given tag is found. If the tag is not found, the stack will popFragment until it is
     * at the root fragment
     *
     * @param transactionOptions Transaction options to be displayed
     */
    public void popFragments(int popDepth, @Nullable FragNavTransactionOptions transactionOptions)
        ...
        if (fragment != null) {
            ...
            if (!mFragmentStacks.get(mSelectedTabIndex).isEmpty()) {
                ...
                ft.replace(mContainerId, fragment, fragment.getTag());
                ft.commit();
            } else {
                fragment = getRootFragment(mSelectedTabIndex);
                ft.replace(mContainerId, fragment, generateTag(fragment));
    ...
    public void popIndexTabFragment(int tab) {
        popIndexTabFragment(tab, 1);
    }

    public void popIndexTabFragment(int tab, int popDepth) {
        popIndexTabFragment(tab, popDepth, null);
    }

    public void popIndexTabFragment(int tab, int popDepth, @Nullable FragNavTransactionOptions transactionOptions)
        throws UnsupportedOperationException {
        if (isRootFragment(tab)) {
            throw new UnsupportedOperationException(
                "You can not popFragment the rootFragment. If you need to change this fragment, use replaceFragment(fragment)");
        } else if (popDepth < 1) {
            throw new UnsupportedOperationException("popFragments parameter needs to be greater than 0");
        ...
        // If our popDepth is big enough that it would just clear the stack, then call that.
        if (popDepth > mFragmentStacks.get(tab).size() - 1) {
            clearStack(transactionOptions);
            return;
        }
        ...
        // Pop the number of the fragments on the stack and remove them from the FragmentManager
        for (int i = 0; i < popDepth; i++) {
            fragment = mFragmentManager.findFragmentByTag(mFragmentStacks.get(tab).pop().getTag());
            if (fragment != null) {
                ft.remove(fragment);
            }
        }
        ...
        if (fragment != null) {
            ...
            if (!mFragmentStacks.get(tab).isEmpty()) {
                fragment = mFragmentStacks.get(tab).peek();
                ft.replace(mContainerId, fragment, fragment.getTag());
                ft.commit();
            } else {
                fragment = getRootFragment(tab);
                ft.replace(mContainerId, fragment, generateTag(fragment));
                ft.commit();
        ...
        // committed to the fragment before we add to the stack
        if (bShouldPush) {
            mFragmentStacks.get(tab).push(fragment);
        }
    ...
    /**
     * Helper function to make sure that we are starting with a clean slate and to perform our first fragment
     * interaction.
     *
    ...
    /**
     * Helper function to get the root fragment for a given index. This is done by either passing them in the
     * constructor, or dynamically via NavListener.
     *
     * @param index The tab index to get this fragment from
     * @return The root fragment at this index
     * @throws IllegalStateException This will be thrown if we can't find a rootFragment for this index. Either because
     *         you didn't provide it in the constructor, or because your RootFragmentListener.getRootFragment(index)
     *         isn't returning a fragment for this index.
     */
    @NonNull
    @CheckResult
    private Fragment getRootFragment(int index) throws IllegalStateException {
        ...
        if (fragment == null) {
            throw new IllegalStateException(
                "Either you haven't past in a fragment at this index in your constructor, or you haven't "
    ...
    public Fragment getCurrentFrag() {
        ...
        // if not, try to pull it from the stack
            ...
            if (!fragmentStack.isEmpty()) {
                mCurrentFrag =
                    mFragmentManager.findFragmentByTag(mFragmentStacks.get(mSelectedTabIndex).peek().getTag());
            }
        }
        return mCurrentFrag;
    }
    ...
    /**
     * Private helper function to clear out the fragment manager on initialization. All fragment management should be
     * done via FragNav.
     */
    ...
    /**
     * @return If true, you are at the bottom of the stack (Consider using replaceFragment if you need to change the
     *         root fragment for some reason) else you can popFragment as needed as your are not at the root
     */
    @CheckResult
    ...
    @CheckResult
    public boolean isRootFragment(int tab) {
        Stack<Fragment> stack = getStack(tab);
     ...
     * @param rootFragments List of root fragments from which to initialize empty stacks. If null, pull fragments from
    ...
    public static final class Builder {
        ...
        /**
         * @param rootFragment A single root fragment. This library can still be helpful when managing a single stack of
         *        fragments
         */
        ...
        /**
         * @param rootFragments a list of root fragments. root Fragments are the root fragments that exist on any tab
         *        structure. If only one fragment is sent in, fragnav will still manage transactions
         */
        ...
        /**
         * @param val A listener to be implemented (typically within the main activity) to fragment transactions
        ...
        public FragNavController build() {
            if (mRootFragmentListener == null && mRootFragments == null) {
                throw new IndexOutOfBoundsException(
                    "Either a root fragment(s) needs to be set, or a fragment listener");
            }
